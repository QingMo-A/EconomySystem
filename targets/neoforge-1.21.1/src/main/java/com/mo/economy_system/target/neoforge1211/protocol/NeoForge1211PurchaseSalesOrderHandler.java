...
public final class NeoForge1211PurchaseSalesOrderHandler {
    ...
    public static void handle(PurchaseSalesOrderMessage message, IPayloadContext context) {
        context.enqueueWork(() -> {
            ...
            try {
                ...
                InventoryAdapter inventory = new InventoryAdapter(buyer);
                PurchaseSalesOrderOutcome outcome = PurchaseSalesOrderService.execute(message,
                        new PurchaseSalesOrderService.Context(buyer.getUUID(), inventory, inventory,
    ...
    private record RepositoryAdapter(MarketSavedData data) implements PurchaseSalesOrderService.Repository {
        ...
        public SalesOrderRemovalResult removeSalesTransactional(UUID tradeId) { return data.removeSalesTransactional(tradeId); }
    }
    ...
    record InventoryAdapter(ServerPlayer player) implements MarketItemMaterializer, TransactionalInventory {
        ...
        public InventoryInsertionResult insert(Object value,int quantity){ItemStack template=((ItemStack)value).copy();template.setCount(1);Inventory inv=player.getInventory();List<ItemStack> before=inv.items.stream().map(ItemStack::copy).toList();try{
            ...
            if(remaining!=0)return InventoryInsertionResult.failure(restore(before));inv.setChanged();return InventoryInsertionResult.success(()->restore(before));
        }catch(RuntimeException exception){boolean restored=restore(before);EconomySystem.LOGGER.error("Inventory insertion failed buyer={} restored={}",player.getUUID(),restored,exception);return InventoryInsertionResult.failure(restored);}}
