...
/** NeoForge Brigadier adapter for the common red-packet use cases. */
public final class Command_RedPacket {
  private static final Logger LOGGER = LogUtils.getLogger();

  private Command_RedPacket() {}
  ...
  private static int create(
    ...
    try {
      RedPacketService.CreateOutcome outcome =
          NeoForge1211RedPacketRuntime.service(server)
              .create(
                  sender.getUUID(),
  ...
  private static int claim(ServerPlayer player, UUID senderId) {
    ...
    try {
      RedPacketService.ClaimOutcome outcome =
          NeoForge1211RedPacketRuntime.service(server).claim(player.getUUID(), senderId);
  ...
  private static int cancel(ServerPlayer sender) {
    ...
    try {
      RedPacketService.CancelOutcome outcome =
          NeoForge1211RedPacketRuntime.service(server).cancel(sender.getUUID());
