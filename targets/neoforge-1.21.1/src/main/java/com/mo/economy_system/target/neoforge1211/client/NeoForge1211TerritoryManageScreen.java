...
public final class NeoForge1211TerritoryManageScreen extends Screen {
  ...
  public NeoForge1211TerritoryManageScreen(Owned initial, Screen parent) {
    ...
    this.controller = new TerritoryManageController(summary.territoryId(), summary.name(),
  ...
  @Override public void tick() {
    ...
    NeoForge1211SingleTerritoryClientState.Snapshot snapshot =
        NeoForge1211SingleTerritoryClientState.snapshot();
  ...
  public void applyResponse(long requestId, SingleTerritoryDataResponseMessage response) {
    ...
    if (response.kind() == SingleTerritoryDataResponseKind.DATA && response.territory().isPresent()) {
      controller.handle(new TerritoryManageEvent.DataLoaded(requestId,
          response.territory().orElseThrow().authorizedMembers().stream()
              .map(member -> new MemberRow(member.playerId(), member.playerName())).toList()));
    } else {
      controller.handle(new TerritoryManageEvent.DataFailed(requestId, "screen.territory.sync_failed"));
    }
  }

  private void navigate(UiNavigation navigation) {
    if (minecraft == null) return;
    if (navigation instanceof UiNavigation.Back) minecraft.setScreen(parent);
  }
  ...
  @Override public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
    ...
    TerritoryManageView.render(new NeoForge1211UiRenderer(graphics, font), controller.state(), layout,
  ...
  @Override public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
    if (scrollY != 0 && controller.state().totalPages() > 1) {
      controller.handle(new TerritoryManageEvent.Scroll(scrollY < 0 ? 1 : -1));
      return true;
    }
