...
/** NeoForge shell for the common market confirmation state. */
public final class NeoForge1211MarketConfirmScreen extends Screen {
  private final Screen parent; private final Port port = new Port(); private final MarketConfirmController controller;
  public NeoForge1211MarketConfirmScreen(MarketAction action, MarketRow row, Screen parent) { super(Component.translatable("screen.market.confirm.title")); this.parent = parent; controller = new MarketConfirmController(action, row, port); }
  @Override public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) { MarketConfirmLayout.Layout layout = MarketConfirmLayout.calculate(width, height, controller.state()); UiScale scale = layout.scale(); graphics.pose().pushPose(); graphics.pose().scale(scale.value(), scale.value(), 1.0f); MarketConfirmView.render(new NeoForge1211UiRenderer(graphics, font), controller.state(), layout, scale.toVirtualX(mouseX), scale.toVirtualY(mouseY)); graphics.pose().popPose(); super.render(graphics, mouseX, mouseY, partialTick); }
