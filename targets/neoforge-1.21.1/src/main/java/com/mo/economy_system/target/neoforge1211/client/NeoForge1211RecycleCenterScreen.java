...
public final class NeoForge1211RecycleCenterScreen extends Screen {
  ...
  public NeoForge1211RecycleCenterScreen(){this(null);} public NeoForge1211RecycleCenterScreen(Screen parent){super(Component.translatable(EconomyUiRoute.RECYCLE.titleKey()));this.parent=parent;}
  ...
  @Override public void render(GuiGraphics g,int mx,int my,float pt){var l=RecycleCenterLayout.calculate(width,height,controller.state(),new NeoForge1211UiTextMetrics(font));UiScale s=l.scale();NeoForge1211UiRenderer r=new NeoForge1211UiRenderer(g,font);r.fillPhysicalBackground(width,height,RecycleCenterLayout.BACKGROUND_COLOR);g.pose().pushPose();g.pose().scale(s.value(),s.value(),1);RecycleCenterView.render(r,controller.state(),l,s.toVirtualX(mx),s.toVirtualY(my));g.pose().popPose();super.render(g,mx,my,pt);}
  @Override public boolean mouseClicked(double mx,double my,int button){var l=RecycleCenterLayout.calculate(width,height,controller.state(),new NeoForge1211UiTextMetrics(font));int x=l.scale().toVirtualX(mx),y=l.scale().toVirtualY(my);for(var row:l.rows())if(row.rect().contains(x,y)){controller.handle(new RecycleCenterEvent.Selected(row.offer().itemId()));return true;}if(l.minus().contains(x,y)){controller.handle(new RecycleCenterEvent.AmountChanged(controller.state().amount()-1));return true;}if(l.plus().contains(x,y)){controller.handle(new RecycleCenterEvent.AmountChanged(controller.state().amount()+1));return true;}if(l.all().contains(x,y)){var o=controller.state().selected();if(o!=null)controller.handle(new RecycleCenterEvent.AmountChanged(o.maxSubmitAmount()));return true;}if(l.submit().contains(x,y)){controller.handle(new RecycleCenterEvent.ActionClicked(RecycleCenterAction.SUBMIT));return true;}if(l.back().contains(x,y)){onClose();return true;}if(controller.state().screenState()==ScreenState.ERROR&&l.retry().contains(x,y)){controller.handle(new RecycleCenterEvent.ActionClicked(RecycleCenterAction.RETRY));return true;}return super.mouseClicked(mx,my,button);}
  ...
  @Override public void onClose(){if(minecraft!=null)minecraft.setScreen(parent!=null?parent:new NeoForge1211HomeScreen());}
  @Override public boolean isPauseScreen(){return false;} @Override public void renderBackground(GuiGraphics g,int x,int y,float p){}
