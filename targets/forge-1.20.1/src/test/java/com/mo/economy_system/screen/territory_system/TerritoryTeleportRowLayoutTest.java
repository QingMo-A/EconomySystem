...
class TerritoryTeleportRowLayoutTest {
  ...
  @Test
  void ownedRowsHaveSeparateTeleportAndInviteHitboxes() {
    var rows =
        List.of(
    ...
    assertEquals(TerritoryTeleportRowLayout.Action.MEMBERS, invite.action());
    ...
    assertTrue(invite.x() + invite.width() <= delete.x());
