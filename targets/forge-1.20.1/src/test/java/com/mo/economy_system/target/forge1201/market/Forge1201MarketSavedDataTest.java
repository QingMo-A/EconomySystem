...
class Forge1201MarketSavedDataTest {
  ...
  void persistsRevisionDefaultsOldSavesAndRejectsNegative() {
    CompoundTag old = root();
    assertEquals(0, MarketSavedData.load(old).getView().revision());
    CompoundTag tagged = root();
    tagged.putLong("marketRevision", 41);
    MarketSavedData data = MarketSavedData.load(tagged);
    assertEquals(41, data.getView().revision());
    assertEquals(41, data.save(new CompoundTag()).getLong("marketRevision"));
    tagged.putLong("marketRevision", -1);
    assertThrows(IllegalArgumentException.class, () -> MarketSavedData.load(tagged));
  }
  ...
  void roundTripsStableOrderAndPreservesDemandAlongsideNewSale() {
    ...
    assertEquals(
        Set.of(MarketOrderType.SALES, MarketOrderType.DEMAND),
        new HashSet<>(
            MarketSavedData.load(saved).getOrders().stream().map(MarketOrder::type).toList()));
  }

  @Test
  void unresolvedLegacyRecordRemainsPendingAndBlocksSave() {
    CompoundTag legacy = new CompoundTag();
    legacy.putString("type", "com.mo.economy_system.core.economy_system.market.SalesOrder");
    legacy.put("itemStack", new CompoundTag());
    CompoundTag root = root(legacy);
    MarketSavedData data = MarketSavedData.load(root);
  ...
  @Test
  void deliveredTransitionPersistsWithoutDuplicatingMixedOrders() {
    MarketOrder demand = order(MarketOrderType.DEMAND, false), sale = sale();
    MarketSavedData data = MarketSavedData.load(root(demand, sale));
    assertEquals(
    ...
    assertEquals(
        ...
        data.markDemandDeliveredIfUnchanged(demand.tradeId(), demand).status());
    MarketSavedData loaded = MarketSavedData.load(data.save(new CompoundTag()));
    assertEquals(2, loaded.getOrders().size());
    MarketOrder updated =
        loaded.getOrders().stream()
            .filter(o -> o.tradeId().equals(demand.tradeId()))
            .findFirst()
            .orElseThrow();
    assertTrue(updated.delivered());
    assertEquals(demand.expirationTime(), updated.expirationTime());
  ...
  private static CompoundTag root(CompoundTag order) {
    CompoundTag root = new CompoundTag();
    ListTag list = new ListTag();
    list.add(order.copy());
    root.put("marketItems", list);
  ...
  private static MarketOrder sale() {
    return order(MarketOrderType.SALES, false);
  }

  private static MarketOrder demand() {
    return order(MarketOrderType.DEMAND, true);
  }

  private static MarketOrder order(MarketOrderType type, boolean delivered) {
    return new MarketOrder(
        type, UUID.randomUUID(), item(), 2, 10, "seller", UUID.randomUUID(), 10, 99, delivered);
  }
