...
class Forge1201CreateDemandOrderProtocolTest {
    @Test void roundTripsProtocolNine() {
        ...
        Forge1201CreateDemandOrderCodec.encode(message,buffer);assertEquals(message,Forge1201CreateDemandOrderCodec.decode(buffer));assertEquals(EconomyProtocol.CREATE_DEMAND_ORDER.discriminator(),EconomyMessages.CREATE_DEMAND_ORDER.discriminator());
    }
    ...
    @Test void bridgeUsesCanonicalDemandCreationDiscriminator() {
        assertEquals(EconomyMessages.CREATE_DEMAND_ORDER.discriminator(), EconomyProtocol.CREATE_DEMAND_ORDER.discriminator());
    }
