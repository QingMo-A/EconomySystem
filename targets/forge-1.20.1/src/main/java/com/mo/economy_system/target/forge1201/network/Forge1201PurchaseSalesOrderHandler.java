...
final class Forge1201PurchaseSalesOrderHandler {
    ...
    static void handle(PurchaseSalesOrderMessage message, Supplier<NetworkEvent.Context> supplier){NetworkEvent.Context network=supplier.get();ServerPlayer buyer=network.getSender();if(buyer!=null)try{
        ...
        PurchaseSalesOrderOutcome outcome=PurchaseSalesOrderService.execute(message,new PurchaseSalesOrderService.Context(buyer.getUUID(),new InventoryAdapter(buyer),new AccountsAdapter(accounts,buyer),new RepositoryAdapter(market),reporter()));
    ...
    private record RepositoryAdapter(MarketSavedData data)implements PurchaseSalesOrderService.Repository{public MarketOrder find(UUID id){return data.getOrder(id);}public SalesOrderRemovalResult removeSalesForPurchase(UUID id){return data.removeSalesForPurchase(id);}}
