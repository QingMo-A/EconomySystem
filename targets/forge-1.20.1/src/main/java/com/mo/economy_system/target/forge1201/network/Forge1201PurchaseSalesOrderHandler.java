...
final class Forge1201PurchaseSalesOrderHandler {
  ...
  static void handle(PurchaseSalesOrderMessage m, Supplier<NetworkEvent.Context> s) {
    NetworkEvent.Context n = s.get();
    ServerPlayer buyer = n.getSender();
    if (buyer != null) execute(m, buyer);
    n.setPacketHandled(true);
  }

  private static void execute(PurchaseSalesOrderMessage m, ServerPlayer buyer) {
    ...
    try {
      ...
      Forge1201TransactionalInventoryAdapter inventory =
          new Forge1201TransactionalInventoryAdapter(buyer);
      o =
    ...
    IsolatedPostActions.runAll(
        MarketActionPostPlan.build(
            ...
            o.result() == PurchaseSalesOrderResult.SUCCESS,
            () -> Forge1201MarketInvalidation.broadcast(buyer),
            () -> feedback(buyer, o),
  ...
  private static Object displayName(ServerPlayer p, MarketOrder o) {
    try {
      ...
      LOGGER.error("Purchase display item restore failed tradeId={}", o.tradeId(), e);
  ...
  private static void sendGenericFailure(ServerPlayer buyer, UUID tradeId) {
    try {
      ...
      LOGGER.error("Sales purchase infrastructure feedback failed tradeId={}", tradeId, exception);
    }
  }
