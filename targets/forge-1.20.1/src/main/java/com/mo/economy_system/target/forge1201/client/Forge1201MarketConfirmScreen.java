...
public final class Forge1201MarketConfirmScreen extends Screen {
  ...
  @Override public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) { MarketConfirmLayout.Layout layout = MarketConfirmLayout.calculate(width, height, controller.state()); UiScale scale = layout.scale(); graphics.pose().pushPose(); graphics.pose().scale(scale.value(), scale.value(), 1.0f); MarketConfirmView.render(new Forge1201UiRenderer(graphics, font), controller.state(), layout, scale.toVirtualX(mouseX), scale.toVirtualY(mouseY)); graphics.pose().popPose(); super.render(graphics, mouseX, mouseY, partialTick); }
