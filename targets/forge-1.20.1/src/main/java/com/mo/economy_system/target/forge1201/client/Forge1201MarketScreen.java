...
public final class Forge1201MarketScreen extends Screen {
  ...
  public Forge1201MarketScreen(Screen parent) { super(Component.translatable(EconomyUiRoute.MARKET.titleKey())); this.parent = parent; Minecraft current = Minecraft.getInstance(); UUID viewer = current == null || current.player == null ? null : current.player.getUUID(); controller = new MarketController(viewer, port); }
  @Override protected void init() { String value = search == null ? "" : search.getValue(); MarketLayout.Layout layout = commonLayout(); UiScale scale = layout.scale(); search = new EditBox(font, Math.round(layout.search().x() * scale.value()), Math.round(layout.search().y() * scale.value()), Math.max(1, Math.round(layout.search().width() * scale.value())), Math.max(1, Math.round(layout.search().height() * scale.value())), Component.translatable("screen.market.search")); search.setMaxLength(64); search.setValue(value); search.setResponder(text -> controller.handle(new MarketEvent.QueryChanged(text))); addRenderableWidget(search); if (controller.state().screenState() == ScreenState.IDLE) controller.handle(new MarketEvent.Initialize(System.nanoTime())); }
  ...
  @Override public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) { MarketLayout.Layout layout = commonLayout(); UiScale scale = layout.scale(); graphics.pose().pushPose(); graphics.pose().scale(scale.value(), scale.value(), 1.0f); MarketView.render(new Forge1201UiRenderer(graphics, font), controller.state(), layout, controller.viewerId(), scale.toVirtualX(mouseX), scale.toVirtualY(mouseY)); graphics.pose().popPose(); super.render(graphics, mouseX, mouseY, partialTick); }
  @Override public boolean mouseClicked(double mouseX, double mouseY, int button) { MarketLayout.Layout layout = commonLayout(); int x = layout.scale().toVirtualX(mouseX), y = layout.scale().toVirtualY(mouseY); for (MarketLayout.Card card : layout.cards()) if (card.actionButton().contains(x, y)) { MarketAction action = MarketView.actionFor(card.row().order(), controller.viewerId()); if (action != null) controller.handle(new MarketEvent.ActionClicked(action, card.row().order().tradeId())); return true; } if (layout.createSales().contains(x, y)) { controller.handle(new MarketEvent.ActionClicked(MarketAction.CREATE_SALES, null)); return true; } if (layout.createDemand().contains(x, y)) { controller.handle(new MarketEvent.ActionClicked(MarketAction.CREATE_DEMAND, null)); return true; } if (controller.state().screenState() == ScreenState.ERROR && layout.message().contains(x, y)) { controller.handle(new MarketEvent.Retry(System.nanoTime())); return true; } if (layout.previousButton().contains(x, y)) { controller.handle(new MarketEvent.PreviousPage()); return true; } if (layout.nextButton().contains(x, y)) { controller.handle(new MarketEvent.NextPage()); return true; } if (layout.esc().contains(x, y)) { controller.handle(new MarketEvent.ActionClicked(MarketAction.BACK, null)); return true; } return super.mouseClicked(mouseX, mouseY, button); }
  ...
  private MarketLayout.Layout commonLayout() { MarketLayout.Layout layout = MarketLayout.calculate(width, height, controller.state()); if (layout.pageSize() != controller.state().pageSize()) { controller.handle(new MarketEvent.ViewportChanged(layout.pageSize())); layout = MarketLayout.calculate(width, height, controller.state()); } return layout; }
