...
/** Forge shell for the common territory invite directory. */
public final class Forge1201TerritoryInviteScreen extends Screen {
  ...
  public Forge1201TerritoryInviteScreen(Owned territory, Screen parent) {
    super(Component.translatable("screen.invite.title"));
    this.territory = territory;
    this.parent = parent;
    UUID viewer = Minecraft.getInstance().player == null
        ? territory.summary().ownerId() : Minecraft.getInstance().player.getUUID();
    var players = territory.authorizedMembers().stream().map(member -> member.playerId()).collect(java.util.stream.Collectors.toSet());
    ...
    controller = new TerritoryInviteController(territory.summary().territoryId(), territory.summary().name(),
        territory.summary().ownerId(), viewer, new HashSet<>(players), port);
  }
  ...
  @Override public void tick() {
    super.tick();
    clientTick++;
    controller.handle(new TerritoryInviteEvent.Tick(System.nanoTime()));
  ...
  @Override public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
    ...
    TerritoryInviteView.render(new Forge1201UiRenderer(graphics, font), controller.state(), layout,
        scale.toVirtualX(mouseX), scale.toVirtualY(mouseY), clientTick);
  ...
  @Override public boolean mouseClicked(double mouseX, double mouseY, int button) {
    ...
    for (var row : layout.playerRows()) if (row.inviteButton().contains(x, y)) {
      controller.handle(new TerritoryInviteEvent.InviteClicked(row.player().playerId(), clientTick));
      return true;
    }
    ...
    if (controller.state().screenState() == ScreenState.ERROR && layout.retryButton().contains(x, y)) {
      controller.handle(new TerritoryInviteEvent.Retry(System.nanoTime())); return true;
    }
  ...
  @Override public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
    if (delta != 0 && controller.state().totalPages() > 1) {
      controller.handle(new TerritoryInviteEvent.Scroll(delta < 0 ? 1 : -1)); return true;
    }
  ...
  private TerritoryInviteLayout.Layout commonLayout() {
    var layout = TerritoryInviteLayout.calculate(width, height, controller.state());
    if (layout.pageSize() != controller.state().pageSize()) {
      controller.handle(new TerritoryInviteEvent.ViewportChanged(layout.pageSize()));
      layout = TerritoryInviteLayout.calculate(width, height, controller.state());
    }
    return layout;
  }
