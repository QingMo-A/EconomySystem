...
/** Forge shell for the common shop page. */
public final class Forge1201ShopScreen extends Screen {
  ...
  public Forge1201ShopScreen() { this(null); }
  ...
  @Override protected void init() {
    ...
    ShopLayout.Layout layout = commonLayout(); UiScale scale = layout.scale();
  ...
  @Override public void tick() {
    super.tick(); controller.handle(new ShopEvent.Tick(System.nanoTime()));
    ClientShopState.Snapshot snapshot = ClientShopState.snapshot();
    if (snapshot.revision() != appliedRevision && port.requestId >= 0) {
      appliedRevision = snapshot.revision(); controller.handle(new ShopEvent.DataLoaded(port.requestId, snapshot.items()));
    }
    controller.pollNavigation().ifPresent(this::navigate);
  }
  private void navigate(UiNavigation navigation) {
    if (minecraft == null) return;
    if (navigation instanceof UiNavigation.Route route && route.route() == EconomyUiRoute.HOME) minecraft.setScreen(new Forge1201HomeScreen());
    else if (navigation instanceof UiNavigation.Back) onClose();
  }
  @Override public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
    ShopLayout.Layout layout = commonLayout(); UiScale scale = layout.scale(); graphics.pose().pushPose(); graphics.pose().scale(scale.value(), scale.value(), 1.0f);
    ShopView.render(new Forge1201UiRenderer(graphics, font), controller.state(), layout, scale.toVirtualX(mouseX), scale.toVirtualY(mouseY)); graphics.pose().popPose(); super.render(graphics, mouseX, mouseY, partialTick);
  }
  @Override public boolean mouseClicked(double mouseX, double mouseY, int button) {
    ShopLayout.Layout layout = commonLayout(); int x = layout.scale().toVirtualX(mouseX), y = layout.scale().toVirtualY(mouseY);
    for (ShopLayout.Card card : layout.cards()) if (card.buyButton().contains(x, y)) { controller.handle(new ShopEvent.ActionClicked(ShopAction.BUY, card.row().item().shopItemId())); return true; }
  ...
  @Override public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
    if (delta != 0 && controller.state().totalPages() > 1) { controller.handle(new ShopEvent.Scroll(delta < 0 ? 1 : -1)); return true; }
  ...
  private ShopLayout.Layout commonLayout() { ShopLayout.Layout layout = ShopLayout.calculate(width, height, controller.state()); if (layout.pageSize() != controller.state().pageSize()) { controller.handle(new ShopEvent.ViewportChanged(layout.pageSize())); layout = ShopLayout.calculate(width, height, controller.state()); } return layout; }
  private final class Port implements ShopPort {
    private long requestId = -1;
    @Override public long nextRequestId() { long id = IDS.getAndIncrement(); if (id < 0) throw new IllegalStateException("shop request id exhausted"); return id; }
    @Override public void requestCatalog(long id) { requestId = id; EconomyServices.platform().network().sendToServer(ShopDataRequestMessage.INSTANCE); }
    @Override public void submit(ShopAction action, ShopRow row, int quantity) { if (action == ShopAction.BUY) EconomyServices.platform().network().sendToServer(new ShopBuyItemMessage(row.item().shopItemId(), quantity)); }
    @Override public void confirm(ShopRow row) { if (minecraft != null) minecraft.setScreen(new Forge1201ShopPurchaseScreen(row, Forge1201ShopScreen.this)); }
  }
}
