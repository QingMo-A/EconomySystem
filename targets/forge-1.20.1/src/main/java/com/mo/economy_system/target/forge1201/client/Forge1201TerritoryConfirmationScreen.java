...
/** Forge shell for shared territory destructive-operation confirmation. */
public final class Forge1201TerritoryConfirmationScreen extends Screen {
  private final Screen parent; private final TerritoryConfirmationController controller;
  public Forge1201TerritoryConfirmationScreen(TerritoryConfirmationKind kind,UUID territoryId,String territoryName,UUID memberId,String memberName,Screen parent){super(Component.translatable("screen.territory.confirm.remove_title"));this.parent=parent;controller=new TerritoryConfirmationController(kind,territoryId,territoryName,memberId,memberName,new Port());}
  @Override public void render(GuiGraphics graphics,int mouseX,int mouseY,float partialTick){TerritoryConfirmationLayout.Layout layout=TerritoryConfirmationLayout.calculate(width,height,controller.state());UiScale scale=layout.scale();Forge1201UiRenderer renderer=new Forge1201UiRenderer(graphics,font);renderer.fillPhysicalBackground(width,height,TerritoryConfirmationLayout.BACKGROUND_COLOR);graphics.pose().pushPose();graphics.pose().scale(scale.value(),scale.value(),1);TerritoryConfirmationView.render(renderer,controller.state(),layout,scale.toVirtualX(mouseX),scale.toVirtualY(mouseY));graphics.pose().popPose();super.render(graphics,mouseX,mouseY,partialTick);}
