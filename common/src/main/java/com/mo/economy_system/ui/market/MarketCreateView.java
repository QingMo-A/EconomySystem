...
public final class MarketCreateView {
  ...
  public static void render(EconomyUiRenderer renderer, MarketCreateState state,
    ...
    UiPanel.render(renderer, layout.formPanel(),
        state.mode() == MarketCreateMode.SALES ? EconomyUiTheme.MARKET_ACCENT : EconomyUiTheme.SHOP_ACCENT,
        2, false);
    if (state.mode() == MarketCreateMode.SALES) {
      ...
      for (MarketCreateLayout.Slot slot : layout.slots()) {
        ...
        boolean selected = slot.hasItem() && slot.slot() == state.selectedSlot();
        UiItemSlot.State slotState = selected
            ? UiItemSlot.State.SELECTED
            : (hovered ? UiItemSlot.State.HOVERED
                : (slot.hasItem() ? UiItemSlot.State.NORMAL : UiItemSlot.State.EMPTY));
        if (slot.hasItem()) {
          UiRect itemRect = new UiRect(slot.rect().x() + 4, slot.rect().y() + 4, 16, 16);
          UiItemSlot.renderItem(renderer, slot.rect(), itemRect,
              slot.item().itemId(), slot.item().count(), slotState, EconomyUiTheme.MARKET_ACCENT);
        } else {
          UiItemSlot.render(renderer, slot.rect(), slotState, EconomyUiTheme.MARKET_ACCENT);
        }
      }
    ...
    if (state.mode() == MarketCreateMode.SALES) {
      renderer.button(layout.decrement(), EconomyUiTheme.MARKET_FORM_BUTTON, "-1", layout.decrement().contains(mouseX, mouseY), state.can(MarketCreateAction.DECREMENT));
      renderer.button(layout.increment(), EconomyUiTheme.MARKET_FORM_BUTTON, "+1", layout.increment().contains(mouseX, mouseY), state.can(MarketCreateAction.INCREMENT));
      renderer.translatedButton(layout.all(), EconomyUiTheme.MARKET_FORM_BUTTON, "screen.market.create.all", List.of(), layout.all().contains(mouseX, mouseY), state.can(MarketCreateAction.SELECT_ALL));
    }
    UiRect totalPreview = new UiRect(layout.price().right() + 10, layout.price().y(),
        Math.max(1, layout.formPanel().right() - 12 - (layout.price().right() + 10)), layout.price().height());
    renderer.translatedTextInRect("screen.market.create.total_preview",
        List.of(UiNumbers.formatInteger(state.computedOrderTotalPrice())), totalPreview,
        state.computedOrderTotalFitsInt() ? EconomyUiTheme.TEXT_SECONDARY : EconomyUiTheme.TEXT_ERROR,
        UiTextAlignment.LEFT);
    renderer.translatedButton(layout.submit(), EconomyUiTheme.MARKET_FORM_BUTTON, "screen.market.create.submit", List.of(), layout.submit().contains(mouseX, mouseY), state.can(MarketCreateAction.SUBMIT));
  ...
  public static void renderCompletionOverlay(EconomyUiRenderer renderer, MarketCreateState state,
    ...
    UiPanel.render(renderer, dropdown, EconomyUiTheme.MARKET_ACCENT, 2, false);
    for (int i = 0; i < count; i++) {
      ...
      UiSection.render(renderer, row,
          suggestionIndex == completionSelection ? EconomyUiTheme.MARKET_ACCENT : 0, hovered);
