...
public final class TerritoryRemovalRateLimiter {
  ...
  public synchronized boolean acquire(UUID player,long tick){Objects.requireNonNull(player);reset(tick);Long prior=attempts.get(player);if(prior!=null&&tick-prior<20)return false;attempts.remove(player);attempts.put(player,tick);while(attempts.size()>capacity)attempts.remove(attempts.keySet().iterator().next());return true;}
