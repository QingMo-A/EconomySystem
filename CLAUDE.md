...
- 虚拟货币（金币）系统，包含商店、市场和交易
- 领地 claiming 和管理，带有增益效果
...
- 数据包在 `EconomySystem_NetworkManager.register()` 中注册
...
- **SQLite + HikariCP** - 数据库和连接池
- **SpongePowered Mixin** - 代码注入
- **Gson** - JSON 序列化
...
4. 在 `EconomySystem_NetworkManager.register()` 中注册数据包
