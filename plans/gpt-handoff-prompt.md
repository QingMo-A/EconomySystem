...
Final lifecycle hardening counts are 435 shared-source test methods, 516 Forge tests, and 576 NeoForge tests; both target suites and `buildAllTargets` pass. The client transfer now captures arrival sessions before queueing, uses a secure relative temp directory and shared budget, rejects new READY while an artifact is pending, routes save/discard only through the coordinator, retries failed deletes, and uses bounded terminal/UI translation keys. Protocol 31+ remains legacy.
