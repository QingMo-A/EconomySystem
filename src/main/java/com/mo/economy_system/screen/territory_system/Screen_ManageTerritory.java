...
public class Screen_ManageTerritory extends Screen {
  ...
  private void renderActionPanel(GuiGraphics guiGraphics, float mouseX, float mouseY) {
    ...
    int contentHeight =
        actionEntries.size() * ACTION_BTN_HEIGHT + (actionEntries.size() - 1) * ACTION_BTN_SPACING;
    int headerHeight = font.lineHeight + 14;
    int panelHeight = Math.max(120, headerHeight + contentHeight + 8);

    CardRenderer.drawCard(
        guiGraphics, panelX, panelY, panelWidth, panelHeight, CardRenderer.THEME_TERRITORY, false);
    ...
    for (ActionEntry entry : actionEntries) {
      ...
      drawStripedButton(
          ...
          hovered);
      actionButtons.add(
          new ButtonArea(panelInnerX, btnY, btnWidth, ACTION_BTN_HEIGHT, entry.onClick()));
    ...
    for (int i = startIndex; i < endIndex; i++) {
      ...
      boolean cardHovered =
          (mouseX >= listX
              ...
              && mouseY >= cardY
              && mouseY <= cardY + PLAYER_CARD_HEIGHT);
      CardRenderer.drawCard(
          guiGraphics,
          listX,
          cardY,
      ...
      String uuidText =
          CardRenderer.truncateText(
              font, playerInfo.getUuid().toString(), listWidth - textX - KICK_BTN_WIDTH - 20);
      ...
      boolean kickHovered =
          (mouseX >= kickX
              && mouseX <= kickX + KICK_BTN_WIDTH
              && mouseY >= kickY
              && mouseY <= kickY + ACTION_BTN_HEIGHT);
      drawStripedButton(
          guiGraphics,
          kickX,
          kickY,
          KICK_BTN_WIDTH,
          ACTION_BTN_HEIGHT,
          Component.translatable(Util_MessageKeys.TERRITORY_MANAGEMENT_KICK_PLAYER).getString(),
          kickStyle,
          kickHovered);
  ...
  public boolean mouseClicked(double mouseX, double mouseY, int button) {
    ...
    for (ButtonArea actionArea : actionButtons) {
      if (virtualMouseX >= actionArea.x()
          && virtualMouseX <= actionArea.x() + actionArea.width()
          && virtualMouseY >= actionArea.y()
          && virtualMouseY <= actionArea.y() + actionArea.height()) {
    ...
    for (ButtonArea kickArea : kickButtons) {
      if (virtualMouseX >= kickArea.x()
          && virtualMouseX <= kickArea.x() + kickArea.width()
          && virtualMouseY >= kickArea.y()
          && virtualMouseY <= kickArea.y() + kickArea.height()) {
        ...
        new ActionEntry(
            ...
            () -> {
              ...
              this.minecraft.player.sendSystemMessage(
                  Component.translatable(Util_MessageKeys.TERRITORY_MANAGEMENT_COPY_SUCCESS));
            }));
        ...
        new ActionEntry(
            ...
            () -> {
              Minecraft.getInstance()
                  .setScreen(
                      new Screen_TerritoryPlayerAction(
                          territory, Screen_TerritoryPlayerAction.Mode.PERMISSION));
            }));
        ...
        new ActionEntry(
            ...
            () -> {
              Minecraft.getInstance()
                  .setScreen(
                      new Screen_TerritoryPlayerAction(
                          territory, Screen_TerritoryPlayerAction.Mode.TRANSFER));
            }));
              ...
              Minecraft.getInstance()
                  .setScreen(
                      new Screen_ConfirmTerritoryRemoval(
                          territory.getTerritoryID(), territory.getName(), this));
            }));
  }
