...
@OnlyIn(Dist.CLIENT)
public class Screen_AuthLogin extends Screen {
    private static final int BG_COLOR = 0xB0202020;      // 半透明黑色背景
    private static final int BORDER_COLOR = 0xFFDD00;    // 黄色边框
    ...
    public Screen_AuthLogin(boolean requireRegistration) {
    ...
    private void updatePromptMessage() {
        if (requireRegistration) {
            statusMessage = Component.literal("§e欢迎首次登录！请设置密码");
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        // 边框
        guiGraphics.fill(RenderType.gui(), boxX, boxY, boxX + boxWidth, boxY + 1, BORDER_COLOR);
        guiGraphics.fill(RenderType.gui(), boxX, boxY + boxHeight - 1, boxX + boxWidth, boxY + boxHeight, BORDER_COLOR);
        guiGraphics.fill(RenderType.gui(), boxX, boxY, boxX + 1, boxY + boxHeight, BORDER_COLOR);
        guiGraphics.fill(RenderType.gui(), boxX + boxWidth - 1, boxY, boxX + boxWidth, boxY + boxHeight, BORDER_COLOR);
    ...
    private void onSubmit() {
        ...
        if (requireRegistration) {
            ...
            ClientAuthHandler.sendRegisterRequest(password);
        } else {
            // 已注册，直接登录
            statusMessage = Component.literal("§a正在登录...");
            messageColor = 0xFF55FF55;
            ClientAuthHandler.sendLoginRequest(password);
        }
    }
