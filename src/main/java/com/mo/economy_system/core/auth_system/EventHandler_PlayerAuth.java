...
//            // 检查玩家是否还存活（存活说明只是死亡，不是真的退出）
//            if (!player.isDeadOrDying()) {
//                // 玩家还存活，这是真正的退出，清除登录状态
...
//                }
//            } else {
//                // 玩家已死亡，不清除登录状态，保留用于重生
//                EconomySystem.LOGGER.info("玩家 " + player.getName().getString() + " 死亡，保留登录状态用于重生");
//            }
//        }
//    }
//
...
//            AuthSavedData data = AuthSavedData.getInstance(level);
//
//            // 如果未登录，但已注册，则自动恢复登录状态
//            if (!data.isLoggedIn(player.getUUID()) && data.isRegistered(player.getUUID())) {
//                EconomySystem.LOGGER.info("玩家 " + player.getName().getString() + " 重生，自动恢复登录状态");
//                data.quickLogin(player.getUUID());
//                player.setGameMode(net.minecraft.world.level.GameType.SURVIVAL);
//            } else if (!data.isLoggedIn(player.getUUID())) {
//                // 未登录且未注册，确保保持旁观者模式
//                player.setGameMode(net.minecraft.world.level.GameType.SPECTATOR);
//            } else {
//                // 已登录，确保是生存模式
